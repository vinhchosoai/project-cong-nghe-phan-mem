            ...
            <div style={{
                ...
                <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', gap: '10px' }}>
                        <button onClick={() => router.push('/restaurant')} style={{ padding: '10px 20px', backgroundColor: '#6c757d', border: 'none', borderRadius: '4px', color: '#fff', cursor: 'pointer' }}>Back</button>
                        <button onClick={handleLogout} style={{ padding: '10px 20px', backgroundColor: '#dc3545', border: 'none', borderRadius: '4px', color: '#fff', cursor: 'pointer' }}>Logout</button>
                    </div>
                </div>
            </div>
            <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
                <div style={{ marginBottom: '20px', textAlign: 'right' }}>
                    <button onClick={() => setShowCategoryModal(true)} style={{ padding: '10px 20px', backgroundColor: '#007bff', border: 'none', borderRadius: '4px', color: '#fff', fontSize: '16px', cursor: 'pointer' }}>+ Add Category</button>
                </div>
                {categories.map(cat => (
                    <div key={cat.category_id} style={{ backgroundColor: '#fff', borderRadius: '8px', boxShadow: '0 0 10px rgba(0, 0, 0, 0.1)', marginBottom: '20px', overflow: 'hidden' }}>
                        <div style={{ backgroundColor: '#f9f9f9', padding: '15px 20px', borderBottom: '1px solid #ccc', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <h2 style={{ margin: 0, fontSize: '20px', color: '#333' }}>{cat.name}</h2>
                            <div style={{ display: 'flex', gap: '10px' }}>
                                <button onClick={() => openCreateItemModal(cat.category_id)} style={{ padding: '8px 15px', backgroundColor: '#fff', border: '1px solid #ccc', borderRadius: '4px', cursor: 'pointer' }}>+ Add Item</button>
                                <button onClick={() => handleDeleteCategory(cat.category_id)} style={{ padding: '8px 15px', backgroundColor: 'transparent', border: 'none', color: '#dc3545', cursor: 'pointer' }}>Delete</button>
                            </div>
                        </div>
                        <div style={{ padding: '10px' }}>
                            {cat.items.map(item => (
                                <div key={item.item_id} style={{ padding: '15px', borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                                        ...
                                        <div>
                                            <h3 style={{ margin: '0 0 5px 0', fontSize: '16px', color: !item.is_available ? '#999' : '#333', textDecoration: !item.is_available ? 'line-through' : 'none' }}>{item.name}</h3>
                                            <p style={{ margin: '0 0 5px 0', fontSize: '14px', color: '#007bff', fontWeight: 'bold' }}>${item.price.toFixed(2)}</p>
                                            {item.description && <p style={{ margin: 0, fontSize: '12px', color: '#666' }}>{item.description}</p>}
                                        </div>
                                    </div>
                                    <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                                        <button onClick={() => openEditItemModal(item)} style={{ padding: '6px 12px', backgroundColor: 'transparent', border: 'none', color: '#007bff', cursor: 'pointer' }}>Edit</button>
                                        <button onClick={() => handleDeleteItem(item.item_id)} style={{ padding: '6px 12px', backgroundColor: 'transparent', border: 'none', color: '#dc3545', cursor: 'pointer' }}>Delete</button>
                                        <button onClick={() => toggleItemAvailability(item)} style={{ padding: '6px 12px', backgroundColor: item.is_available ? '#28a745' : '#6c757d', border: 'none', borderRadius: '4px', color: '#fff', cursor: 'pointer', fontSize: '12px' }}>
                                            {item.is_available ? 'Available' : 'Unavailable'}
                                        </button>
                                    </div>
                                </div>
                            ))}
                            {cat.items.length === 0 && <div style={{ padding: '30px', textAlign: 'center', color: '#999' }}>No items in this category</div>}
                        </div>
                    </div>
                ))}
            ...
            {showCategoryModal && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
                    <div style={{ backgroundColor: '#fff', borderRadius: '8px', padding: '30px', width: '100%', maxWidth: '400px' }}>
                        <h2 style={{ margin: '0 0 20px 0', fontSize: '20px' }}>Add Category</h2>
                        <form onSubmit={handleCreateCategory}>
                            ...
                            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                                ...
                                <button type="submit" style={{ padding: '10px 20px', backgroundColor: '#007bff', border: 'none', borderRadius: '4px', color: '#fff', cursor: 'pointer' }}>Add</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            {showItemModal && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
                    <div style={{ backgroundColor: '#fff', borderRadius: '8px', padding: '30px', width: '100%', maxWidth: '500px', maxHeight: '90vh', overflowY: 'auto' }}>
                        <h2 style={{ margin: '0 0 20px 0', fontSize: '20px' }}>{editingItem ? 'Edit Item' : 'Add New Item'}</h2>
                        <form onSubmit={handleItemSubmit}>
                            ...
                            <div style={{ marginBottom: '15px' }}>
                                ...
                                <textarea value={itemForm.description} onChange={e => setItemForm({ ...itemForm, description: e.target.value })} style={{ width: '100%', padding: '10px', border: '1px solid #ccc', borderRadius: '4px', fontSize: '14px', minHeight: '80px' }} />
                            </div>
                            <div style={{ marginBottom: '20px' }}>
                                <label style={{ display: 'block', marginBottom: '5px', fontSize: '14px', fontWeight: 'bold' }}>Image URL</label>
                            ...
                            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                                ...
                                <button type="submit" style={{ padding: '10px 20px', backgroundColor: '#007bff', border: 'none', borderRadius: '4px', color: '#fff', cursor: 'pointer' }}>
